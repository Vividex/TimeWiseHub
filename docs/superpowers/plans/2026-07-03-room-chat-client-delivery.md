# Room Chat + Client Delivery Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a "Chat" tab to the in-call panel (joining Transcript and Program from Phase 1),
scoped to one video call's participants (staff + the client), letting staff actually hand the
client files/links from the linked program — not just show them via screen share.

**Architecture:** Team chat's existing tables/RPC/storage/RLS are reused almost entirely unmodified
— a new `chat_conversations.type = 'session'` value is the only structural addition, plus a
`session_id` column to find it and a `guest_chat_user_id` column on `clients` to give each client
one persistent (non-anonymous, admin-created) identity, reused across every session. The two
panels shipped in Phase 1 (transcript, program) are refactored into one shared tabbed shell so the
new Chat tab has somewhere to live without a third competing slide-in panel.

**Tech Stack:** Next.js 16 App Router (RSC), React 19, TypeScript strict, Tailwind v4, Supabase
(admin API for guest identity, realtime for live messages). No new npm dependencies.

## Global Constraints

- No new npm dependencies.
- Reuse existing chat infrastructure (`send_chat_message` RPC, `chat-attachments` storage bucket,
  `MessageComposer`, `AttachmentChip`, `ChatMessage`/`ChatAttachment` types) rather than building
  parallel mechanisms — only the pieces genuinely new to a session-scoped, guest-inclusive
  conversation get new code.
- Guest identity: one real (admin-created, non-anonymous) `profiles` row per client, stored on
  ...
  Plain Supabase anonymous auth cannot be used here (`profiles.email` is `NOT NULL`, anonymous
  users have no email, the signup trigger would fail).
- Sharing a program asset into chat posts a message referencing it (name + link) — never copies
  the file into chat's own storage bucket.
- Session-type conversations must never appear in the normal Team Chat inbox — reachable only via
  the session detail page and the live call itself.
- Verification gate: `pnpm run build` (tsc + eslint) after every task. No test runner.
- All Tailwind classes must include `dark:` variants where the surrounding UI isn't hard-coded
  dark (the call room UI itself is always dark, matching the existing Phase 1 panels — no `dark:`
  variants needed there, consistent with what's already shipped).
...
**Files:**
- Create: `supabase/schema-077-session-chat-enum.sql`
- Create: `supabase/schema-078-session-chat.sql`

**Interfaces:**
- Produces: `chat_conversation_type` enum gains a `'session'` value; `chat_conversations.session_id`
  (nullable, unique-when-set); `clients.guest_chat_user_id` (nullable); `can_post_chat()` treats
  `'session'` conversations like `'dm'` for posting purposes.

*Conductor only — no Codex dispatch needed, matches this project's established convention for
migrations.*

- [ ] **Step 1: Create the enum-value migration**

Postgres requires a new enum value to be committed before anything in the same transaction can
reference it — this MUST be its own migration, applied and committed before the next one.
...
Use `apply_migration` (name: `session_chat_enum`).

- [ ] **Step 3: Create the structural migration**
...
Use `apply_migration` (name: `session_chat`).

- [ ] **Step 5: Verify via MCP `execute_sql`**
...
```bash
git add supabase/schema-077-session-chat-enum.sql supabase/schema-078-session-chat.sql
git commit -m "feat: room chat + client delivery — database migrations"
```

---

## Task 2: `session-chat.ts` — guest identity and participant helpers

**Files:**
- Create: `src/lib/session-chat.ts`

**Interfaces:**
- Consumes: `createServiceClient` (`@/lib/supabase-service`)
- Produces:
  - `ensureGuestChatUser(clientId: string): Promise<{ userId: string; email: string }>`
  - `mintGuestChatToken(email: string): Promise<string>`
  - `ensureSessionChatParticipant(sessionId: string, userId: string): Promise<string>` (returns
    `conversationId`)
  All consumed by Task 7 (staff wiring) and Task 8 (guest wiring).
...
- [ ] **Step 3: Commit**
  ...
  ```bash
  git add src/lib/session-chat.ts
  git commit -m "feat: room chat + client delivery — session-chat.ts helpers"
  ```

---

## Task 3: `CallPanel` — shared tabbed shell

**Files:**
- Create: `src/components/video/CallPanel.tsx`

**Interfaces:**
- Produces: `CallPanelTabId = 'transcript' | 'program' | 'chat'`; `export default function
  CallPanel(props: { open: boolean; activeTab: CallPanelTabId; availableTabs: CallPanelTabId[];
  onSelectTab: (tab: CallPanelTabId) => void; onClose: () => void; children: React.ReactNode })`
  — consumed by Task 6.
...
- [ ] **Step 3: Commit**
  ...
  ```bash
  git add src/components/video/CallPanel.tsx
  git commit -m "feat: room chat + client delivery — CallPanel shared tabbed shell"
  ```

---

## Task 4: Refactor `ProgramReferencePanel` — content-only + share-to-chat

**Files:**
- Modify: `src/components/programs/ProgramReferencePanel.tsx`

**Interfaces:**
- Consumes: `LinkedProgramBundle`, `ProgramAsset`, `ProgramAssetType` (`@/types/programs`)
- Produces: `export default function ProgramReferencePanel(props: { linkedProgram:
  LinkedProgramBundle; sessionChat: { conversationId: string } | null })` — note `linkedProgram` is
  now non-nullable and `open`/`onClose` are gone (the parent `CallPanel`, Task 3, owns
  visibility/closing now). Consumed by Task 6.

- [ ] **Step 1: Read the current file, then replace its full contents**

Read `src/components/programs/ProgramReferencePanel.tsx` first to confirm it matches the version
shipped in Phase 1, then replace its full contents:
...
- [ ] **Step 2: Verify build passes**
  ...
  pnpm run build
  ```
  Expected: a type error in `CallRoom.tsx` — it still renders the old `<ProgramReferencePanel
  linkedProgram={...} open={...} onClose={...} />` shape. This is expected here, fixed by Task 6.

- [ ] **Step 3: Commit**

  ```bash
  git add src/components/programs/ProgramReferencePanel.tsx
  git commit -m "feat: room chat + client delivery — ProgramReferencePanel content-only + share-to-chat"
  ```

---

## Task 5: `RoomChatTab` — chat tab content

**Files:**
- Create: `src/components/video/RoomChatTab.tsx`

**Interfaces:**
- Consumes: `createClient` (`@/lib/supabase-browser`), `AttachmentChip`
  (`@/components/chat/AttachmentChip`), `MessageComposer` (`@/components/chat/MessageComposer`),
  `ChatMessage` type (`@/lib/chat/types`).
- Produces: `export default function RoomChatTab(props: { conversationId: string; userId: string
  })` — consumed by Task 6.
...
- [ ] **Step 3: Commit**
  ...
  ```bash
  git add src/components/video/RoomChatTab.tsx
  git commit -m "feat: room chat + client delivery — RoomChatTab component"
  ```

---

## Task 6: Refactor `CallRoom` — unified tabbed panel

**Files:**
- Modify: `src/components/video/CallRoom.tsx`

**Interfaces:**
- Consumes: `CallPanel`/`CallPanelTabId` (Task 3), `ProgramReferencePanel` (Task 4), `RoomChatTab`
  (Task 5).
- Produces: `Props` gains `sessionChat?: { conversationId: string; userId: string } | null` —
  consumed by Task 7 and Task 8.

- [ ] **Step 1: Read the current file, then replace its full contents**

Read `src/components/video/CallRoom.tsx` first to confirm it matches the version shipped in
Phase 1, then replace its full contents:
...
- [ ] **Step 2: Verify build passes**
  ...
  pnpm run build
  ```
  Expected: Compiled successfully, 0 TypeScript errors — the Task 4 mismatch is now resolved.
  `linkedProgram`/`sessionChat` being optional means `GuestJoinClient.tsx`'s existing `<CallRoom
  ... />` call (which doesn't pass either) keeps compiling unchanged.

- [ ] **Step 3: Commit**

  ```bash
  git add src/components/video/CallRoom.tsx
  git commit -m "feat: room chat + client delivery — CallRoom unified tabbed panel"
  ```

---

## Task 7: Wire staff chat participancy on the call page

**Files:**
- Modify: `src/app/dashboard/video/[roomId]/page.tsx`

**Interfaces:**
- Consumes: `ensureSessionChatParticipant` (Task 2)
- Produces: `<CallRoom ... sessionChat={sessionChat} />`

- [ ] **Step 1: Read the current file, then apply this change**

Read `src/app/dashboard/video/[roomId]/page.tsx` first to confirm it matches the version shipped
in Phase 1. Add the import:

```typescript
import { ensureSessionChatParticipant } from '@/lib/session-chat'
```

Then, after the existing `const linkedProgram = call.session_id ? await fetchLinkedProgram(call.session_id, user.id) : null`
line, add:
...
Then update the final `<CallRoom ... />` render to add `sessionChat={sessionChat}` alongside the
existing `linkedProgram={linkedProgram}` prop:
...
- [ ] **Step 3: Commit**
  ...
  git commit -m "feat: room chat + client delivery — staff chat participancy on the call page"
  ```

---

## Task 8: Wire guest chat identity and sign-in

**Files:**
- Modify: `src/app/api/video/token/route.ts`
- Modify: `src/app/join/[guestToken]/page.tsx`
- Modify: `src/components/video/GuestJoinClient.tsx`

**Interfaces:**
- Consumes: `ensureGuestChatUser`, `mintGuestChatToken`, `ensureSessionChatParticipant` (Task 2)
- Produces: guest browser gets a real Supabase session via `verifyOtp`; `<CallRoom ...
  sessionChat={...} />` for guests.

- [ ] **Step 1: Read `src/app/api/video/token/route.ts` first, then apply this change**
...
- [ ] **Step 2: Read `src/app/join/[guestToken]/page.tsx` first, then apply this change**

Add `client_id` to the invitee query isn't needed here (the token route resolves everything) — no
change needed in this file. Confirm it still passes `guestToken` through to `GuestJoinClient`
unchanged (it already does).

- [ ] **Step 3: Read `src/components/video/GuestJoinClient.tsx` first, then replace its full contents**
...
Note: if `verifyOtp` fails for any reason, `sessionChat` simply stays `null` — the guest still
joins the video call normally, just without the Chat tab (same "never block the video join over a
chat problem" principle as the server-side `try/catch` in Step 1).
...
- [ ] **Step 5: Commit**
  ...
  ```bash
  git add "src/app/api/video/token/route.ts" src/components/video/GuestJoinClient.tsx
  git commit -m "feat: room chat + client delivery — guest chat identity and sign-in"
  ```

---

## Task 9: Exclude session-type conversations from Team Chat + fix notification URL

**Files:**
- Modify: `src/components/chat/ChatRealtimeProvider.tsx`
- Modify: `src/lib/chat/notify.ts`

**Interfaces:** none new — both are self-contained fixes to existing code paths.

- [ ] **Step 1: Read `src/components/chat/ChatRealtimeProvider.tsx` first, then apply this change**

Find the `loadConversations` function:
...
- [ ] **Step 2: Read `src/lib/chat/notify.ts` first, then apply this change**
...
- [ ] **Step 4: Commit**
  ...
  ```bash
  git add src/components/chat/ChatRealtimeProvider.tsx src/lib/chat/notify.ts
  git commit -m "feat: room chat + client delivery — exclude session chats from Team Chat inbox, fix notification link"
  ```

---

## Task 10: "Call chat" review section on the session detail page
...
**Interfaces:**
- Produces: `export default function SessionCallChat(props: { conversationId: string })` —
  consumed by `SessionDetailClient`.
...
- [ ] **Step 3: Commit**
  ...
  ```bash
  git add src/components/clients/SessionCallChat.tsx
  git commit -m "feat: room chat + client delivery — SessionCallChat review component"
  ```

- [ ] **Step 4: Read `src/app/dashboard/clients/[id]/sessions/[sessionId]/page.tsx` first, then
  apply this change**

After the existing `let call: ...` block (which fetches `scheduled_calls` for the video-call
summary), add a lookup for the session's chat conversation:
...
Then add `sessionChatConversationId={sessionChatConversationId}` to the `<SessionDetailClient
...
- [ ] **Step 5: Verify build passes**
  ...
  pnpm run build
  ```
  Expected: a type error on `SessionDetailClient` (doesn't accept `sessionChatConversationId` yet)
  — expected here, fixed by Step 6.

- [ ] **Step 6: Read `src/components/clients/SessionDetailClient.tsx` first, then apply this
  change**
...
Add `sessionChatConversationId: string | null` to the destructured props and the type signature,
...
(the one that closes the `space-y-2` block containing the Notes `ChevronDown` button and textarea)
and before that column's own closing `</div>`, add:
...
- [ ] **Step 8: Commit**
  ...
  git commit -m "feat: room chat + client delivery — Call Chat section on session detail page"
  ```
...
- [ ] **Step 2: Manual browser smoke test** (no test runner in this project — this feature touches
  live auth, so this pass matters more than usual):
  1. Open a session with a linked program, ensure the client has an email on file, schedule/join
     its video call as staff. Confirm Notes / Program / Chat buttons all appear, each opening the
     same panel on their own tab; clicking the active tab's button again closes the panel.
  ...
  4. Send a message from the guest — confirm it appears live in staff's Chat tab (and vice versa).
  ...
  9. Re-join the SAME session's call a second time (e.g. schedule a follow-up call for the same
     session, or rejoin) as the same client guest — confirm they're recognised as the same person
     (no duplicate guest account created) and can see the same chat history.
...
- [ ] Task 1: migrations applied, verified via `execute_sql`, committed
- [ ] Task 2: `session-chat.ts` compiles clean, all three helpers match the spec's guest-identity
  and participant-upsert design
- [ ] Task 3: `CallPanel` compiles clean, matches the approved tabbed-shell design
- [ ] Task 4: `ProgramReferencePanel` is content-only (no own header/wrapper), share-to-chat works
- [ ] Task 5: `RoomChatTab` compiles clean, reuses `MessageComposer`/`AttachmentChip` as designed
- [ ] Task 6: `CallRoom` renders the unified panel, all three tabs wire up correctly, guests never
  receive `linkedProgram`
- [ ] Task 7: staff chat participancy ensured on the call page
- [ ] Task 8: guest identity/sign-in flow works end to end, never blocks video join on failure
- [ ] Task 9: session-type conversations excluded from Team Chat inbox, notification links to the
  session page
- [ ] Task 10: "Call Chat" section appears on the session page when a conversation exists
- [ ] Task 11: full manual smoke test passes, including the guest-isolation and repeat-visit checks

## Verification
`pnpm run build` (next build = tsc + eslint) must pass clean after every task. Manual browser
smoke test required for Task 11 (no test runner in this project) — this feature touches live
Supabase Auth (admin-created users, magic-link sign-in), so the manual pass is the only real
verification that guest identity actually works end to end.
