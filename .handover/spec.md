...
- [ ] T2: Replace `src/components/finance/IncomeList.tsx` with version that has ConfirmDialog, loading state (`deletingId`), and error handling (`setError`)
- [ ] T3: Add `setCurrency('AUD')` and `setCategory('Sales')` to the submit reset block in `src/components/finance/IncomeForm.tsx` (after the existing `setAmount` / `setDescription` / `setDate` resets)
