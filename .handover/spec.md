# Sessions This Week — Dashboard Tile + Overview Page

## Goal
Replace the "Hours this week" tile on the home dashboard with "Sessions this week", linking to a
new org-wide `/dashboard/sessions` page showing this week's sessions and all other scheduled
sessions, each flagged Recurring when applicable.
...
- Week = simple Monday–Sunday calendar week via shared `getWeekBounds()` helper in `src/lib/week.ts`.
- "This week" tile/section = any status, `[weekStart, weekEnd)` against `scheduled_at`.
- New page's "Scheduled" section = `status = 'scheduled'` AND `scheduled_at >= weekEnd`, no cap.
- `hoursThisWeek` and its supporting `time_entries`/`roster_shifts` queries are dead code once the
  tile changes — removed entirely, not left stranded. `todayStart`/`nextWeek`/`now` stay (used by
  the meetings/calendar-events queries later on the same page).
- No inline "New session" creation form on the new page — sessions are always created from a
  specific client's context.
- No spend: this feature is pure code — internal Supabase reads only, no external API calls.
...
## C-1 — Shared week-bounds helper

*Codex edits:*
- [x] Create `src/lib/week.ts`:
  ```typescript
  ...
  ```
  (Exact same Monday-start-of-week calculation already inline in
  `src/app/dashboard/page.tsx` today — extracted so C-2 and C-4 don't duplicate it.)

*Conductor:*
- [x] `pnpm run build` — must pass clean. Nothing imports this yet — checks it compiles standalone.
- [x] Commit: `git add src/lib/week.ts && git commit -m "feat: sessions this week — shared getWeekBounds helper"`
...
- [x] Manual browser smoke test (no test runner):
  ...
  2. On `/dashboard/sessions`, confirm the "This week" section shows sessions scheduled Mon–Sun
     of the current week, and the count in the subtitle matches the tile.
