...
export default function ProjectSiteControl({
  ...
  currentSiteAddress: string | null
}) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 text-sm">
      ...
      {editing ? (
        ...
        <>
          <span className="text-gray-700 dark:text-slate-300">{currentSiteAddress ?? 'No site assigned'}</span>
          <button onClick={() => setEditing(true)} className="text-xs font-semibold text-cyan-600 hover:text-cyan-700 dark:text-cyan-400">
            {currentSiteAddress ? 'Change' : 'Assign site'}
          </button>
        </>
      )}
    </div>
