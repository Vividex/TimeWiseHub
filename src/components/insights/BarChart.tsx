...
export default function BarChart({ days, title }: { days: DayBar[]; title: string }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
      <h3 className="mb-6 text-sm font-bold uppercase tracking-wide text-gray-500">{title}</h3>
      <div className="flex items-end gap-2" style={{ height: '140px' }}>
        {days.map(day => {
          const pct = (day.hours / max) * 100
          return (
            <div key={day.label} className="flex flex-1 flex-col items-center gap-1">
              <span className="text-xs font-semibold text-gray-400" style={{ minHeight: '18px' }}>
